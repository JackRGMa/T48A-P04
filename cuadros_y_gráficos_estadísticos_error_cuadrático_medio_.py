...
pct = np.array([0.688, 0.688, 0.563, 0.250, 0.125, 0.688, 0.625, 0.438,
                0.313, 0.688, 0.625, 0.500, 0.438, 0.125, 0.813, 0.688,
                0.563, 0.375, 0.250, 0.688, 0.563, 0.313, 0.313, 0.313,
                0.875, 0.750, 0.375, 0.375])

# Crear el histograma
plt.hist(pct, bins=5, edgecolor='black', color='gray')
plt.title('Histograma de Porcentajes de Juegos Ganados')
...
# Calcular las frecuencias absoluta (contar la cantidad de datos que hay en cada clase)
hist, bins = np.histogram(pct, bins=5)

# Frecuencia relativa
#La frecuencia relativa es la proporción de datos que caen dentro de una clase específica, comparado con el total de datos.
#Se calcula dividiendo la frecuencia absoluta de cada clase entre el total de datos.
frecuencia_relativa = hist / len(pct)
...
# Crear el polígono de frecuencias
plt.plot(bin_centers, hist, marker='o', color='cyan', linestyle='-.')
plt.title('Polígono de Frecuencias de Porcentaje de Juegos Ganados')
plt.xlabel('Porcentaje')
plt.ylabel('Frecuencia')
plt.grid(True)
plt.show()
...
# Calcular la frecuencia acumulada e invierte el histograma, ya que la frecuencia se muestra de forma descendente
frecuencia_acumulada = np.flip(np.cumsum(np.flip(hist)))

# Calcular los puntos medios de los bins
bin_centers = (bins[:-1] + bins[1:]) / 2

# Crear la ojiva menor que
plt.plot(bin_centers, frecuencia_acumulada, marker='o', color='purple', linestyle='--')
plt.title('Ojiva Mayor que de Porcentaje de Juegos Ganados')
plt.xlabel('Porcentaje')
plt.ylabel('Frecuencia Acumulada descendente')
plt.grid(True)
plt.show()
